...
//Grab the methods
    ...
    public User getUserGivenUsername(String username) {
        ...
        //try(Connection conn = DriverManager.getConnection(this.url,this.username,this.password)){ instead of conn.close
        try{

            Connection conn = ConnectionUtil.getConnection();

            // ? dynamically insert a value
            String sql = "SELECT * FROM users WHERE user_username = ?;";

            //preparing our sql statement prevents sequel injections to delete tables
            PreparedStatement ps = conn.prepareStatement(sql);

            //we are adding the username into the question mark in the sql statement
            ps.setString(1, username);

            //for all DQL statements
            ...
            conn.close(); //save memory or data
